...
import { getOptionChainTool, getOptionExpiries, getMaxPainTool, getFnoLotsTool, } from "./tools/options.js";
import { downloadDocument } from "./tools/document.js";
import { getBoardMeetings } from "./tools/boardMeetings.js";
import { getBseQuote, getBseGainers, getBseLosers, getBseAdvanceDecline, getBseResultCalendar, } from "./tools/bseMarket.js";
...
export function createMcpServer() {
    const server = new Server({ name: "trade-vision", version: "0.1.0" }, { capabilities: { tools: {} } });
    // -------------------------------------------------------------------------
    // Tool definitions
    // -------------------------------------------------------------------------
    server.setRequestHandler(ListToolsRequestSchema, async () => ({
        tools: [
            {
                name: "get_bulk_deals",
                description: "Fetch today's NSE bulk deals — trades where a single client buys or sells ≥0.5% of a company's listed shares in one session. Returns the buyer/seller, quantity, and weighted-average price.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: {
            ...
            },
            {
                ...
                description: "Fetch today's NSE block deals — large negotiated trades (minimum ₹10 crore) executed in the Block Deal Window. Shows counterparty, quantity, and trade price.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: {
            ...
            },
            {
                ...
                description: "Fetch SEBI PIT (Prohibition of Insider Trading) disclosures — promoter and insider buy/sell transactions reported to the exchanges. Shows acquirer name, category, shares before/after, and mode of acquisition.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: {
            ...
            },
            {
                ...
                description: "Convenience wrapper — returns today's bulk deals sorted by trade value, optionally filtered by direction.",
                inputSchema: {
                    type: "object",
                    properties: {
                        dealType: {
            ...
            },
            {
                ...
                description: "Today's largest bulk-deal purchases ranked by total trade value (quantity × price). Useful for spotting institutional accumulation.",
                inputSchema: {
                    type: "object",
                    properties: {
                        limit: {
            ...
            },
            {
                ...
                description: "Today's largest bulk-deal sales ranked by total trade value. Useful for spotting institutional distribution.",
                inputSchema: {
                    type: "object",
                    properties: {
                        limit: {
            ...
            },
            {
                ...
                description: "Daily FII (Foreign Institutional Investor) and DII (Domestic Institutional Investor) net buy/sell figures in the Indian cash market. Key macro signal for market direction.",
                inputSchema: {
                    type: "object",
                    properties: {
                        limit: {
            ...
            },
            {
                ...
                description: "Corporate announcements filed with NSE — board meetings, results, dividends, mergers, regulatory updates, etc. Equivalent to 8-K filings on SEC EDGAR.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: {
            ...
            },
            {
                ...
                description: "Current NSE market status — whether Capital Market, F&O, Currency Derivatives, and Commodity segments are open or closed, plus live index levels.",
                inputSchema: {
                    type: "object",
                    properties: {},
                },
            },
            {
                ...
                description: "Live data for all NSE indices (Nifty 50, Nifty Bank, Nifty IT, Nifty Midcap 150, etc.) — price, change, 52-week range, advances/declines.",
                inputSchema: {
                    type: "object",
                    properties: {
                        name: {
                            type: "string",
                            description: "Filter by index name (partial, case-insensitive). E.g. \"BANK\" returns all bank indices.",
                        },
                    },
                },
            },
            {
                ...
                description: "Top gaining NSE stocks by percentage change for the current session within a given index.",
                inputSchema: {
                    type: "object",
                    properties: {
                        index: {
                            type: "string",
                            description: "Market segment to scan. Maps to an NSE category (NIFTY, BANKNIFTY, NIFTYNEXT50, FOSec). Default: all securities.",
                        },
                        limit: {
                            type: "number",
                            description: "Number of results to return. Default: 10.",
            ...
            {
                name: "get_top_losers",
                description: "Top losing NSE stocks by percentage change for the current session within a given index.",
                inputSchema: {
                    type: "object",
                    properties: {
                        index: {
            ...
            },
            {
                ...
                description: "Most actively traded NSE stocks by total traded value (₹) in the current session.",
                inputSchema: {
                    type: "object",
                    properties: {
                        index: {
            ...
            },
            {
                ...
                description: "Aggregate view for a single NSE stock — returns today's bulk deals, block deals, recent insider trades, and recent announcements in one call.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: {
                        ...
                        daysBack: {
                            type: "number",
                            description: "How many days back to look for insider trades and announcements. Default: 30.",
                        },
                    },
                    required: ["symbol"],
                },
            ...
            {
                name: "get_quote",
                description: "Live stock quote from Yahoo Finance for any NSE-listed stock. Includes price, 52-week range, volume, market cap, P/E ratios, dividend yield, and next earnings date. Prices are in INR.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: {
                            type: "string",
                            description: "NSE symbol (e.g. RELIANCE, TCS). The .NS exchange suffix is appended automatically. Pass RELIANCE.BO to query BSE instead.",
                        },
                    },
                    required: ["symbol"],
                },
            ...
            },
            {
                ...
                description: "Technical screener catalog and scoring engine for NSE stocks. Supports momentum, breakout, intraday, swing, mean-reversion, volatility, sector-rotation, and accumulation-style screens. Returns a ranked list of candidates with a trader-style reason and supporting signal context.",
                inputSchema: {
                    type: "object",
                    properties: {
                        screenType: {
            ...
            },
            {
                ...
                description: "NSE short-selling data — quantity of shares sold short and second-leg (covering) quantities, as reported by brokers under SEBI's short-selling framework.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: {
                        ...
                        limit: {
                            type: "number",
                            description: "Return top N stocks by short-sold quantity. Omit for all.",
                        },
                    },
                },
            },
            {
                ...
                description: "Upcoming and recent corporate actions — dividends, stock splits, bonus issues, rights issues, and buy-backs. Shows ex-date, record date, and payment date.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: {
            ...
            },
            {
                ...
                description: "List NSE/BSE IPOs with grey-market premium, subscription multiples, issuer financials and an Apply/Skip/Watch signal. Status: current (open now), upcoming, latest (recently closed), closed, or all. Each row carries the computed signal + reasons.",
                inputSchema: {
                    type: "object",
                    properties: {
                        status: {
            ...
            },
            {
                ...
                description: "Full detail for a single IPO — price band, issue size, lot size, GMP, subscription by category (QIB/NII/retail), issuer financials, key dates, and the weighted Apply/Skip/Watch signal with every contributing check and reason. Identify the IPO by symbol or company name.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: {
            ...
            {
                name: "get_past_ipos",
                description: "Past NSE/BSE IPOs within an optional date window (filters latest + closed listings by listing/close date). Returns the same enriched Apply/Skip/Watch records as the calendar.",
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            },
            {
                ...
                description: "Best-effort DRHP/RHP prospectus links for an IPO by scanning NSE's IPO detail payload for .pdf URLs.",
                inputSchema: {
                    type: "object",
            ...
            },
            {
                ...
                description: "Historical India VIX (volatility index) daily closes between two dates. Useful for regime/fear context.",
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            },
            {
                ...
                description: "Full NSE option chain for a symbol with computed analytics — total CE/PE OI, PCR (put-call ratio), max pain, and per-strike OI. Option-chain section needs browser-like headers + cookie priming.",
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            },
            {
                ...
                description: "Fetch a document (DRHP, RHP, circular, report) and return metadata — content-type, a text snippet for inline payloads, or a link for binary (PDF) files. Caller supplies the URL.",
                inputSchema: {
                    type: "object",
            ...
            {
                name: "get_iv_radar",
                description: "Implied-volatility analytics for an NSE F&O stock from the live option chain + realised-vol history: ATM IV, IV rank & percentile (realised-vol proxy), risk reversal, and the volatility regime (ELEVATED/NORMAL/COMPRESSED).",
                inputSchema: {
                    type: "object",
                    properties: {
            ...
            },
            {
                ...
                description: "Options positioning & dealer-hedging pressure for an NSE F&O stock: max pain, gamma wall (OI-weighted approximation), expected move (straddle), squeeze targets, and call-resistance / put-support pressure zones.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: { type: "string", description: "NSE F&O symbol (e.g. RELIANCE)." },
            ...
            },
            {
                ...
                description: "Geometric-Brownian-motion Monte Carlo of the future price (default 10,000 paths over 30 trading days) estimated from trailing daily returns. Returns mean/median price, 68% and 90% ranges, probability above spot, probability of a 10% drop, and a terminal-price distribution.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: { type: "string", description: "NSE symbol (e.g. RELIANCE)." },
            ...
            },
            {
                ...
                description: "Native backtest of long-only strategies over trailing daily bars (buy & hold, SMA 50/200 cross, RSI 14 mean-reversion, MACD 12/26/9 cross). Returns cumulative/CAGR return, Sharpe, Sortino, max drawdown, win rate, and trade count per strategy.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: { type: "string", description: "NSE symbol (e.g. RELIANCE)." },
            ...
            },
            {
                ...
                description: "Pre-trade risk scan for a single NSE stock: liquidity score, ATR volatility, suggested stop-loss, position sizing from a risk budget, gap risk, earnings proximity, implied move, and a PROCEED / REDUCE_SIZE / AVOID recommendation with flags.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: { type: "string", description: "NSE symbol (e.g. RELIANCE)." },
            ...
            },
            {
                ...
                description: "Generate three native SVG charts for a symbol — candlestick price, implied-volatility-by-strike, and an options-flow (OI) heatmap — written to a local charts/ directory. Returns the file paths and the inline SVG markup.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: { type: "string", description: "NSE symbol (e.g. RELIANCE)." },
            ...
            },
            {
                ...
                description: "Structured markdown research note synthesising quote, technicals, chart patterns, options/volatility, the AI read, Monte Carlo, backtest, fundamentals, and the aggregate verdict into one report.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: { type: "string", description: "NSE symbol (e.g. RELIANCE)." },
            ...
            },
            {
                ...
                description: "Institutional-style aggregate verdict for an NSE stock: combines the AI directional read, IV radar, option pressure, Monte Carlo, and backtest into a STRONG BUY → STRONG SELL signal with a confidence score and the supporting bullish/bearish factors.",
                inputSchema: {
                    type: "object",
                    properties: {
                        symbol: { type: "string", description: "NSE symbol (e.g. RELIANCE)." },
            ...
            switch (name) {
                ...
                case "get_stock_screener": {
                    const symbols = Array.isArray(a.symbols)
                        ? a.symbols.filter((item) => typeof item === "string")
                        : undefined;
                    result = {
                        registry: getScreenerRegistry(),
                        output: await runScreener({
                            screenType: strEnum("screenType"),
                            symbols,
                            limit: num("limit"),
                            sector: str("sector"),
                ...
                case "get_bse_quote": {
                    ...
                    if (!scrip)
                        throw new Error("scrip is required");
