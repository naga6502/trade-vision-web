import { fetchBars, ema, rsi, adx, atr, bollingerWidth, detectCandlePatterns, } from "./technical.js";
// ---------------------------------------------------------------------------
...
export function getScreenerRegistry() {
...
async function computeMetrics(symbolRaw) {
    ...
    if (cached && Date.now() - cached.at < METRICS_TTL_MS)
        return cached.m;
    const bars = await fetchBars(ticker, "1d", 400).catch(() => []);
    let m = null;
    if (bars.length >= 60) {
        ...
        const rangePos = last.high > last.low
            ...
            : 50;
        const candlePats = detectCandlePatterns(bars.slice(-3), {}).filter((p) => p.type === "BULLISH");
        m = {
            symbol: ticker,
            price,
            prevClose,
...
function classify(m) {
    const trend = m.price > m.ema50 && (Number.isNaN(m.ema200) || m.ema50 >= m.ema200)
        ? "UP"
        : m.price < m.ema200
            ? "DOWN"
            : "SIDEWAYS";
    const momentum = m.rsi >= 60 ? "STRONG" : m.rsi <= 40 ? "WEAK" : "NEUTRAL";
    const volume = m.volRatio >= 1.5 ? "HIGH" : m.volRatio >= 1 ? "NORMAL" : "LOW";
    let risk = "MEDIUM";
...
const evaluators = {
    momentum(m) {
        if (!Number.isFinite(m.ema20) || !Number.isFinite(m.ema50))
            return null;
        const reasons = [];
        let s = 0;
        if (m.price > m.ema20) {
        ...
        if (Number.isFinite(m.ema200) && m.ema50 > m.ema200) {
            s += 8;
            reasons.push("EMA50 > EMA200");
        }
        if (m.rsi >= 55) {
            s += 10;
            reasons.push(`RSI ${m.rsi.toFixed(0)}`);
        }
        else if (m.rsi >= 50)
            s += 5;
        if (Number.isFinite(m.adx) && m.adx > 20 && m.plusDI > m.minusDI) {
            s += 12;
            reasons.push(`ADX ${m.adx.toFixed(0)}`);
        }
        if (m.volRatio >= 1.2) {
            s += 8;
            reasons.push(`vol ${m.volRatio.toFixed(1)}x`);
        }
        if (m.changePct > 0)
            s += 6;
        if (s < 30)
    ...
    breakout(m) {
        if (!Number.isFinite(m.ema20) || m.high20 <= 0)
            return null;
        const reasons = [];
        let s = 0;
        ...
        if (above) {
            s += 35;
            reasons.push("new 20D high");
        }
        else if (near) {
            s += 22;
            reasons.push(`${pct(m.price, m.high20).toFixed(1)}% to 20D high`);
        }
        else
            return null;
        if (m.volRatio >= 1.5) {
        ...
        else if (m.volRatio >= 1.1)
            s += 12;
        if (m.price > m.ema20)
            s += 12;
        if (Number.isFinite(m.adx) && m.adx > 20) {
            s += 10;
            reasons.push(`ADX ${m.adx.toFixed(0)}`);
        }
        if (m.changePct > 0)
    ...
    swing(m) {
        if (!Number.isFinite(m.ema50))
            return null;
        const reasons = [];
        let s = 0;
        if (m.price > m.ema50) {
            s += 12;
            reasons.push("above EMA50 (uptrend)");
        }
        if (Number.isFinite(m.ema200) && m.price > m.ema200)
            s += 6;
        const inZone = m.price <= m.ema20 * 1.03 && m.price >= m.ema50 * 0.98;
        if (inZone) {
            s += 30;
            reasons.push("pulled back to MAs");
        }
        else if (m.price <= m.ema20 * 1.06)
            s += 12;
        else
            return null;
        if (m.bullishReversal) {
            s += 25;
            reasons.push(`reversal: ${m.patterns[0]}`);
        }
        if (m.volRatio >= 1)
            s += 12;
        if (m.changePct > -1 && m.changePct < 4)
            s += 10;
        if (s < 35)
            return null;
        return buildEval(m, { base: s, reason: reasons, pattern: m.patterns[0] });
    ...
    "mean-reversion"(m) {
        ...
        if (m.rsi <= 35) {
            s += 35;
            reasons.push(`RSI ${m.rsi.toFixed(0)} oversold`);
        }
        else if (m.rsi <= 42) {
            s += 18;
            reasons.push(`RSI ${m.rsi.toFixed(0)}`);
        ...
        if (near200) {
            s += 18;
            reasons.push("near 200EMA support");
        }
        if (nearLow) {
            s += 14;
            reasons.push("near 20D low");
        }
        if (m.lastBullish) {
            s += 18;
            reasons.push("bounce candle");
        }
        else if (m.changePct < -3)
            return null;
        if (m.volRatio >= 0.9)
            s += 10;
        if (m.changePct > -1.5)
    ...
    volatility(m) {
        ...
        if (bbExp) {
            s += 35;
            reasons.push("Bollinger widening");
        }
        if (atrExp) {
            s += 30;
            reasons.push("ATR rising");
        }
        if (!bbExp && !atrExp)
            return null;
    ...
    accumulation(m) {
        ...
        if (m.price > m.ema20)
            s += 10;
        if (m.price > m.close5ago) {
            s += 20;
            reasons.push("rising price");
        }
        if (m.volAvgLast5 > m.volAvgPrev5 * 1.05) {
            s += 30;
            reasons.push("volume expanding");
        }
        else if (m.volAvgLast5 > m.volAvgPrev5)
            s += 12;
        if (m.lastBullish && m.volCurrent >= m.volAvg20) {
            s += 25;
            reasons.push("accumulation day");
        }
        if (m.volRatio >= 1)
            s += 15;
        if (s < 35)
    ...
    "sector-rotation"(m) {
        ...
        if (m.ret20 >= 5) {
            s += 25;
            reasons.push(`${m.ret20.toFixed(1)}% / 20D`);
        }
        else if (m.ret20 >= 0)
            s += 10;
        else
        ...
        if (Number.isFinite(m.adx) && m.adx > 20 && m.plusDI > m.minusDI) {
            s += 20;
            reasons.push(`ADX ${m.adx.toFixed(0)} leadership`);
        }
        if (m.volRatio >= 1.2) {
            s += 15;
            reasons.push(`vol ${m.volRatio.toFixed(1)}x`);
        }
        if (Number.isNaN(m.ema200) || m.ema50 >= m.ema200)
            s += 15;
        if (s < 35)
            return null;
        return buildEval(m, { base: s, reason: reasons, pattern: m.patterns[0] });
...
function toResult(m, screenName, e) {
    return {
        symbol: m.symbol,
        screenName,
        score: e.score,
...
function dailyIntradayQualifies(m) {
    if (!Number.isFinite(m.ema20))
        return false;
    if (m.rangePos < 55)
        return false;
    if (m.rsi < 45)
...
async function computeIntradayMetrics(symbolRaw) {
    const raw = (symbolRaw ?? "").trim().toUpperCase();
    if (!raw)
        return null;
    const ticker = /\.[A-Z]{1,3}$/.test(raw) ? raw : `${raw}.NS`;
...
function evaluateIntraday(m, im) {
    const reasons = [];
    let s = 0;
    ...
    if (aboveOrb) {
        s += 35;
        reasons.push(`above ORB high ${im.orbHigh.toFixed(1)}`);
    }
    else if (aboveVwap) {
        s += 18;
        ...
        return null; // below both: no intraday long setup
    if (m.volRatio >= 1.4) {
        s += 25;
        reasons.push(`vol ${m.volRatio.toFixed(1)}x`);
    }
    ...
    if (m.changePct > 0)
        s += 8;
    if (m.price > im.orbHigh * 1.02)
        s -= 5; // too extended from the breakout
    if (s < 35)
        return null;
    return buildEval(m, { base: s, reason: reasons, pattern: m.patterns[0] });
}
async function evalIntradayScreen(metrics, limit) {
...
export async function runScreener(args) {
    ...
    const selected = screenName !== "all" ? screens.filter((s) => s.name === screenName) : screens;
    return {
