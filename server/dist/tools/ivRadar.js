...
import { validateSymbol, fetchDailyBars, annualizedVol, } from "../quant/util.js";
export async function getIvRadar(args) {
    ...
    if (!chain || !chain.strikes.length || !chain.underlying) {
        notes.push("Option chain unavailable (stock may be non-F&O, or NSE blocked programmatic access). IV analytics require an active option chain.");
        return {
            symbol: ticker,
            underlying: null,
            expiry: null,
    ...
    const atmIvParts = [];
    if (atmStrike.ce && atmStrike.ce.impliedVolatility > 0)
        atmIvParts.push(atmStrike.ce.impliedVolatility);
    if (atmStrike.pe && atmStrike.pe.impliedVolatility > 0)
        atmIvParts.push(atmStrike.pe.impliedVolatility);
    ...
    const smile = strikes
        .map((s) => {
        const parts = [];
        if (s.ce && s.ce.impliedVolatility > 0)
            parts.push(s.ce.impliedVolatility);
        if (s.pe && s.pe.impliedVolatility > 0)
            parts.push(s.pe.impliedVolatility);
    ...
    const riskReversal = callIv != null && putIv != null && callIv > 0 && putIv > 0 ? Number((callIv - putIv).toFixed(4)) : null;
    // Realised-vol distribution over the last year for rank/percentile.
    const hist = await fetchDailyBars(ticker, 365).catch(() => ({ ticker, bars: [] }));
    const closes = hist.bars.map((b) => b.close);
    let ivRank = null;
    let ivPercentile = null;
    let realizedVol21d = null;
    if (closes.length >= 60) {
        const window = 21;
        const series = [];
    ...
    if (atmIv != null && realizedVol21d != null) {
        if (atmIv > realizedVol21d * 1.4)
            volatilityRegime = "ELEVATED";
        else if (atmIv < realizedVol21d * 0.7)
...
//# sourceMappingURL=ivRadar.js.map