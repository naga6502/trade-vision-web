...
function leg(o: unknown): OptionLeg | null {
...
function headers(cookie: string, referer: string): Record<string, string> {
  return {
...
async function fetchOptionChain(symbol: string): Promise<any> {
  ...
  const res = await fetch(
    ...
    { headers: headers(cookies, OC_PAGE), signal: AbortSignal.timeout(30000) },
  );
...
function summarize(strikes: OptionStrike[]): OptionChainSummary {
  let totalCE = 0;
  let totalPE = 0;
...
export async function getOptionChain(
  ...
  const expiryDates: string[] = Array.isArray(records.expiryDates)
    ? records.expiryDates
    : [];
  const data: any[] = raw?.filtered?.data ?? records.data ?? [];
...
export async function getExpiryDates(symbol: string): Promise<string[]> {
  const raw = await fetchOptionChain(symbol);
  const expiryDates: string[] = Array.isArray(raw?.records?.expiryDates)
    ? raw.records.expiryDates
    : [];
  return expiryDates;
}

export async function getMaxPain(symbol: string, expiry?: string): Promise<number | null> {
...
export async function getFnoLots(symbol: string): Promise<number | null> {
  const raw = await fetchOptionChain(symbol);
