...
const rupee = (n: number | null | undefined, dp = 2) =>
  n == null || !Number.isFinite(n) ? "n/a" : `₹${n.toLocaleString("en-IN", { maximumFractionDigits: dp })}`;
const pct = (n: number | null | undefined) =>
  n == null || !Number.isFinite(n) ? "n/a" : `${(n as number).toFixed(1)}%`;

export async function generateStockResearchReport(args: ResearchReportArgs): Promise<ResearchReport> {
  ...
  if (quote) {
    L.push(
      `- **Price:** ${rupee(quote.price)} (prev close ${rupee(quote.previousClose)})`,
    );
  ...
  if (patterns.length) {
    for (const row of patterns) {
      const ps = row.patterns.map((p: any) => `${p.name} (${p.type}${p.forming ? ", forming" : ""})`).join(", ");
  ...
  if (eq) {
    ...
    for (const s of eq.strategies) {
      L.push(
        `| ${s.strategy} | ${pct(s.annualizedReturnPct)} | ${s.sharpe} | ${s.sortino} | ${pct(s.maxDrawdownPct)} | ${(s.winRate * 100).toFixed(0)}% | ${s.numTrades} |`,
      );
    }
    L.push("");
  }
  ...
  if (analyze) {
    ...
    if (analyze.bullishFactors.length) L.push("- Bullish: " + analyze.bullishFactors.join("; "));
    if (analyze.bearishFactors.length) L.push("- Bearish: " + analyze.bearishFactors.join("; "));
    L.push("");
  }
