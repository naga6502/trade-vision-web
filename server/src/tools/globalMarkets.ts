...
const DEFAULT_INDICES: { symbol: string; name: string }[] = [
...
let cache: { at: number; data: GlobalIndex[] } | null = null;

export async function getGlobalMarkets(args: GlobalMarketsArgs = {}): Promise<GlobalIndex[]> {
  ...
  if (cache && Date.now() - cache.at < CACHE_TTL_MS) {
    ...
    return list
      ...
      .filter((d): d is GlobalIndex => d != null);
  }

  const data = await Promise.all(
    list.map(async (l): Promise<GlobalIndex> => {
      try {
