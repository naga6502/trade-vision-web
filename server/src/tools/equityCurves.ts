...
// Native backtester. Runs a basket of long-only strategies over trailing daily
// bars (Yahoo Finance) and returns equity/risk metrics. No external backtest
// service. Strategies: buy & hold, SMA(50/200) cross, RSI(14) mean-reversion,
// MACD(12,26,9) cross.
...
function positionSmaCross(closes: number[]): number[] {
  const fast = smaSeries(closes, 50);
  const slow = smaSeries(closes, 200);
  return closes.map((_, i) => (fast[i] > slow[i] ? 1 : 0));
}

function positionRsiMeanReversion(closes: number[]): number[] {
  ...
  for (let i = 0; i < closes.length; i++) {
    ...
    if (state === 0 && r[i] < 30) state = 1;
    else if (state === 1 && r[i] > 70) state = 0;
    pos[i] = state;
  }
  return pos;
}

function positionMacdCross(closes: number[]): number[] {
  const { line, signal } = macdSeries(closes);
  return closes.map((_, i) =>
    Number.isNaN(line[i]) || Number.isNaN(signal[i]) ? 0 : line[i] > signal[i] ? 1 : 0,
  );
}

function positionHold(): (c: number[]) => number[] {
  return (closes: number[]) => closes.map(() => 1);
}

interface BacktestOut {
  metrics: Omit<EquityMetrics, "strategy" | "description">;
  trades: { ret: number }[];
}

function backtest(closes: number[], position: number[]): BacktestOut {
  const equity = [1];
  ...
  for (let i = 1; i < position.length; i++) {
    if (position[i - 1] === 0 && position[i] === 1) entry = i;
    else if (position[i - 1] === 1 && position[i] === 0 && entry >= 0) {
      trades.push({ ret: closes[i] / closes[entry] - 1 });
      entry = -1;
    }
  }
  if (entry >= 0) trades.push({ ret: closes[closes.length - 1] / closes[entry] - 1 });
...
export async function getEquityCurves(args: EquityCurvesArgs): Promise<EquityCurvesResult> {
  ...
  if (bars.length < 60) throw new Error(`Not enough history for ${ticker}`);

  const closes = bars.map((b) => b.close);

  const specs: { name: string; description: string; pos: number[] }[] = [
  ...
  const strategies: EquityMetrics[] = specs.map((s) => {
