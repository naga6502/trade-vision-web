import type { Ipo, IpoSignal, IpoSignalCheck, IpoSignalVerdict } from "./types.js";

const APPLY_THRESHOLD = 25;
const SKIP_THRESHOLD = -10;
const MAX_SCORE = 120;

interface Check extends IpoSignalCheck {
  // status already part of IpoSignalCheck
}
 ...
 */
export function computeIpoSignal(ipo: Ipo): IpoSignal {
  const checks: Check[] = [];
  ...
  let gmpPct: number | null = ipo.gmpPercent;
  if (gmpPct === null && ipo.gmp !== null && band) {
    const mid = (band.min + band.max) / 2;
    if (mid > 0) gmpPct = (ipo.gmp / mid) * 100;
  }
  ...
  // 3. QIB subscription
  if (s?.qib != null) {
    if (s.qib >= 10) add({ id: "sub_qib", label: "QIB subscription", weight: 20, status: "PASS", points: 20, note: `QIB subscribed ${s.qib}x — strong institutional demand.` });
    else if (s.qib >= 3) add({ id: "sub_qib", label: "QIB subscription", weight: 20, status: "WARN", points: 10, note: `QIB subscribed ${s.qib}x.` });
    else if (s.qib < 1) add({ id: "sub_qib", label: "QIB subscription", weight: 20, status: "FAIL", points: -15, note: `QIB under-subscribed (${s.qib}x).` });
    else add({ id: "sub_qib", label: "QIB subscription", weight: 20, status: "WARN", points: 4, note: `QIB subscribed ${s.qib}x.` });
  } else {
  ...
  if (s?.nii != null) {
    if (s.nii >= 20) add({ id: "sub_nii", label: "HNI/NII subscription", weight: 12, status: "PASS", points: 12, note: `HNI subscribed ${s.nii}x.` });
    else if (s.nii >= 5) add({ id: "sub_nii", label: "HNI/NII subscription", weight: 12, status: "WARN", points: 6, note: `HNI subscribed ${s.nii}x.` });
    else if (s.nii < 1) add({ id: "sub_nii", label: "HNI/NII subscription", weight: 12, status: "FAIL", points: -10, note: `HNI under-subscribed (${s.nii}x).` });
    else add({ id: "sub_nii", label: "HNI/NII subscription", weight: 12, status: "WARN", points: 3, note: `HNI subscribed ${s.nii}x.` });
  } else {
  ...
  if (s?.retail != null) {
    if (s.retail >= 5) add({ id: "sub_retail", label: "Retail subscription", weight: 5, status: "PASS", points: 5, note: `Retail subscribed ${s.retail}x.` });
    else if (s.retail < 1) add({ id: "sub_retail", label: "Retail subscription", weight: 5, status: "WARN", points: -5, note: `Retail under-subscribed (${s.retail}x).` });
    else add({ id: "sub_retail", label: "Retail subscription", weight: 5, status: "WARN", points: 0, note: `Retail subscribed ${s.retail}x.` });
  } else {
  ...
  if (rg != null || pg != null) {
    const pos = [rg, pg].filter((v): v is number => v != null && v > 0).length;
    const neg = [rg, pg].filter((v): v is number => v != null && v < 0).length;
    if (neg > 0)
  ...
  if (f?.roe != null) {
    if (f.roe >= 15) add({ id: "fnd_roe", label: "Fundamentals — ROE", weight: 10, status: "PASS", points: 10, note: `ROE ${f.roe}% — healthy returns.` });
    else if (f.roe >= 0) add({ id: "fnd_roe", label: "Fundamentals — ROE", weight: 10, status: "WARN", points: 4, note: `ROE ${f.roe}%.` });
    else add({ id: "fnd_roe", label: "Fundamentals — ROE", weight: 10, status: "FAIL", points: -10, note: `Negative ROE (${f.roe}%).` });
  } else {
  ...
  if (f?.debtEquity != null) {
    if (f.debtEquity <= 0.5) add({ id: "fnd_leverage", label: "Fundamentals — leverage", weight: 8, status: "PASS", points: 8, note: `Low debt/equity (${f.debtEquity}).` });
    else if (f.debtEquity <= 1) add({ id: "fnd_leverage", label: "Fundamentals — leverage", weight: 8, status: "WARN", points: 3, note: `Moderate debt/equity (${f.debtEquity}).` });
    else add({ id: "fnd_leverage", label: "Fundamentals — leverage", weight: 8, status: "FAIL", points: -8, note: `High debt/equity (${f.debtEquity}).` });
  } else {
  ...
  const weakDemand =
    (s?.total != null && s.total < 1) || (s?.qib != null && s.qib < 1);
  const negativeGmp = gmpPct != null && gmpPct <= 0;
  const richVal =
    f?.peRatio != null && f?.industryPe != null && f.industryPe > 0 &&
  ...
  const confidence: "HIGH" | "MEDIUM" | "LOW" =
    completeness === 3 && ipo.priceBand ? "HIGH" : completeness >= 1 ? "MEDIUM" : "LOW";

  let signal: IpoSignalVerdict;
  if (criticalFail) signal = "SKIP";
  else if (score >= APPLY_THRESHOLD) signal = "APPLY";
  else if (score <= SKIP_THRESHOLD) signal = "SKIP";
  else signal = "WATCH";

  const decisive = checks
    .filter((c) => c.status !== "WARN" || c.points !== 0)
    .sort((a, b) => Math.abs(b.points) - Math.abs(a.points));
  const reasons: string[] = decisive.slice(0, 6).map(
    (c) => `[${c.status}] ${c.label}: ${c.note}`
  );
  ...
  return {
    ...
    reasons,
    disclaimer:
      "Heuristic signal for research only — not investment advice. GMP is unofficial grey-market data and is volatile.",
  };
}
