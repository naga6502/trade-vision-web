...
function unixToIso(unixSec) {
    if (unixSec == null)
        throw new Error("Yahoo Finance: missing regularMarketTime");
...
function cleanExchange(node) {
    return typeof node === "string" && EXCHANGE_RE.test(node) ? node : null;
}
function cleanCurrency(node, ticker) {
    if (typeof node === "string" && CURRENCY_RE.test(node))
        return node;
    throw new YahooMalformedResponseError(`Yahoo Finance: invalid or missing currency for ${ticker}`);
}
export class YahooNotFoundError extends Error {
}
export class YahooMalformedResponseError extends Error {
}
export function parseQuoteSummary(json, ticker) {
    const root = json.quoteSummary;
    if (!root) {
        throw new YahooMalformedResponseError(`Yahoo Finance: missing quoteSummary envelope for ${ticker}`);
    }
    if (root.error) {
    ...
    const earningsBlock = (calendar.earnings ?? {});
    const firstEarnings = earningsBlock.earningsDate?.[0];
    return {
        ticker,
        name: typeof price.shortName === "string" ? price.shortName : typeof price.longName === "string" ? price.longName : null,
        exchange: cleanExchange(price.exchange),
        currency: cleanCurrency(price.currency, ticker),
        timestamp: unixToIso(regularMarketTime),
        price: currentPrice,
        previousClose: required(pickRaw(summary.regularMarketPreviousClose) ??
            pickRaw(price.regularMarketPreviousClose), "previousClose", ticker),
...
function required(value, name, ticker) {
    if (value == null) {
