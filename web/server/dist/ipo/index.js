import { fetchNseIpos } from "./nse.js";
...
async function findIpo(symbol, companyName) {
    const statuses = ["current", "upcoming", "latest", "closed"];
    ...
    return (all.find((i) => {
        if (ns && i.symbol && i.symbol.toUpperCase() === ns)
            return true;
        if (nn && i.companyName.toLowerCase().includes(nn))
            return true;
        return false;
    }) ?? null);
}
...
export async function getPastIpos(args = {}) {
 ...
 */
export async function getIpoProspectus(symbol) {
    const sym = symbol.trim().toUpperCase();
    try {
        const data = await fetchNSE(`/api/ipo-details/${encodeURIComponent(sym)}`, {
            ttlMs: 0,
        });
