...
function candlestickSvg(bars) {
    ...
    const range = hi - lo || 1;
    const y = (p) => M.top + plotH - ((p - lo) / range) * plotH;
...
function ivSurfaceSvg(smile, underlying) {
    ...
    const kR = kHi - kLo || 1;
    const vR = vHi - vLo || 1;
    const x = (k) => M.left + ((k - kLo) / kR) * plotW;
    const y = (v) => M.top + plotH - ((v - vLo) / vR) * plotH;
...
function optionsFlowSvg(strikes) {
    ...
    const maxOi = Math.max(1, ...strikes.map((s) => Math.max(s.ce?.openInterest ?? 0, s.pe?.openInterest ?? 0)));
...
export async function generateStockImages(args) {
    ...
    {
        const svg = optionsFlowSvg(chainStrikes.map((s) => ({
