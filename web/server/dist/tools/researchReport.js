...
const NA = "—";
const num = (n, dp = 2) => n == null || !Number.isFinite(n) ? NA : Number(n).toFixed(dp);
const rupee = (n, dp = 2) => n == null || !Number.isFinite(n)
    ? NA
    : `₹${Number(n).toLocaleString("en-IN", { maximumFractionDigits: dp })}`;
const pct = (n) => n == null || !Number.isFinite(n) ? NA : `${Number(n).toFixed(1)}%`;
...
function rupeeCompact(n, dp = 0) {
    if (n == null || !Number.isFinite(n))
        return NA;
    const v = Number(n);
    const abs = Math.abs(v);
    if (abs >= 1e7)
        return `₹${(v / 1e7).toFixed(dp)} Cr`;
    if (abs >= 1e5)
        return `₹${(v / 1e5).toFixed(dp)} L`;
...
export async function generateStockResearchReport(args) {
    ...
    if (tech) {
        L.push(`- **Composite:** ${tech.summary.label} (score ${num(tech.summary.score)}, ${tech.summary.buy} buy / ${tech.summary.neutral} neutral / ${tech.summary.sell} sell)`);
    ...
    if (patterns.length) {
        for (const row of patterns) {
            const ps = row.patterns
                .map((p) => `${p.name} (${titleCase(p.type)}${p.forming ? ", forming" : ""})`)
                .join(", ");
    ...
    if (op || iv) {
        ...
        if (iv) {
            L.push(`- **ATM IV:** ${iv.atmIv != null ? pct(iv.atmIv) : NA} · **Regime:** ${iv.volatilityRegime}`);
            L.push(`- **IV rank / percentile:** ${iv.ivRank ?? NA} / ${iv.ivPercentile ?? NA}`);
            L.push(`- **Risk reversal:** ${iv.riskReversal ?? NA}`);
        }
        if (op) {
            L.push(`- **Max pain:** ${rupee(op.maxPain)} · **Gamma wall:** ${rupee(op.gammaWall?.strike ?? null)}`);
            L.push(`- **Expected move:** ${rupee(op.expectedMove?.points ?? null)} (${pct(op.expectedMove?.pct ?? null)}) to ${op.expiryDate ?? NA}`);
            if (op.pressureZones.length) {
                L.push("- **Pressure zones:** " +
                    op.pressureZones
    ...
    if (!ai && !mc)
        L.push("- No probabilistic model output.");
    L.push("");
    // Backtest
    ...
    if (eq) {
        ...
        for (const s of eq.strategies) {
            L.push(`| ${s.strategy} | ${pct(s.annualizedReturnPct)} | ${num(s.sharpe, 2)} | ${num(s.sortino, 2)} | ${pct(s.maxDrawdownPct)} | ${(s.winRate * 100).toFixed(0)}% | ${s.numTrades} |`);
        }
        L.push("");
    }
    // Fundamentals
    if (fundamentals) {
        ...
        L.push(`- **Leverage:** D/E ${num(fundamentals.debtToEquity, 2)}, current ratio ${num(fundamentals.currentRatio, 2)}`);
        L.push(`- **Growth:** revenue ${pct(fundamentals.revenueGrowth)}, earnings ${pct(fundamentals.earningsGrowth)}`);
        L.push(`- **Analyst:** mean ${num(fundamentals.recommendationMean, 1)} (1=strong buy) · target ${rupee(fundamentals.targetMeanPrice)}`);
        L.push("");
    }
    // Verdict
    if (analyze) {
        L.push("## Verdict");
        L.push(`**${analyze.signal}** · composite ${num(analyze.score, 2)} · confidence ${Math.round(analyze.confidenceScore * 100)}%`);
