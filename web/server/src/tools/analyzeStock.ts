...
// Aggregate "analyze stock" verdict: combines the AI directional read, IV radar,
// option-pressure, Monte Carlo, and backtest into a single bull/bear call with
// a confidence score and the supporting bullish/bearish factors. All inputs are
// computed natively (Yahoo Finance + NSE); see individual tools for method notes.
...
function verdictFromScore(s: number): Verdict {
  ...
  if (s >= 0.2) return "BUY";
  if (s <= -0.6) return "STRONG SELL";
  if (s <= -0.2) return "SELL";
  return "NEUTRAL";
}

export async function getAnalyzeStock(args: AnalyzeStockArgs): Promise<AnalyzeStock> {
  ...
  if (ai) {
    ...
    wsum += w;
    if (ai.prediction === "UP") bullish.push(`AI ensemble: UP (up-prob ${(ai.upProbability * 100).toFixed(0)}%, strength ${ai.signalStrength})`);
    else if (ai.prediction === "DOWN") bearish.push(`AI ensemble: DOWN (down-prob ${((1 - ai.upProbability) * 100).toFixed(0)}%, strength ${ai.signalStrength})`);
    else bullish.push("AI ensemble: FLAT / no edge");
  }
  ...
  if (mc) {
    ...
    wsum += w;
    if (p > 0.55) bullish.push(`Monte Carlo: ${(p * 100).toFixed(0)}% prob above spot, median ₹${mc.medianPrice}`);
    else if (p < 0.45) bearish.push(`Monte Carlo: ${(p * 100).toFixed(0)}% prob above spot, median ₹${mc.medianPrice}`);
  }
  ...
  if (pressure && pressure.maxPain != null) {
    ...
    else bearish.push(`Option max pain ₹${mp} is below spot — gravitational drag into expiry`);
  }
  ...
  if (iv && iv.riskReversal != null) {
    ...
    wsum += w;
    if (iv.riskReversal > 0.03) bullish.push(`Call skew (risk reversal +${iv.riskReversal}) — upside demand`);
    else if (iv.riskReversal < -0.03) bearish.push(`Put skew (risk reversal ${iv.riskReversal}) — downside demand`);
  }
  ...
  if (eq && eq.strategies.length) {
    ...
    wsum += w;
    if (best.sharpe > 1) bullish.push(`Best backtest (${best.strategy}) Sharpe ${best.sharpe}, CAGR ${best.annualizedReturnPct}%`);
    else if (best.sharpe < 0) bearish.push(`Backtests unprofitable (best Sharpe ${best.sharpe})`);
  }
  ...
  if (ai) confParts.push(ai.confidence);
  if (mc) confParts.push(clamp(Math.abs(mc.probAboveSpot - 0.5) * 2, 0, 1));
  const availability = (Number(!!iv) + Number(!!pressure) + Number(!!eq)) / 3;
  if (confParts.length) confParts.push(availability * 0.5);
  const confidenceScore = confParts.length ? round(confParts.reduce((a, b) => a + b, 0) / confParts.length, 3) : 0;

  const summary =
    `${ticker} reads ${signal} (composite ${finalScore.toFixed(2)}, confidence ${(confidenceScore * 100).toFixed(0)}%). ` +
