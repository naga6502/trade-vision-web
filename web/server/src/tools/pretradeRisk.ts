...
// Pre-trade risk scan for a single NSE stock, computed natively from Yahoo
// quote history + NSE option chain. Surfaces liquidity, volatility, stop-loss,
// position sizing, gap/earnings proximity, and an execution recommendation.
...
export async function getPretradeRiskScan(args: PretradeRiskArgs): Promise<PretradeRiskScan> {
  ...
  if (currentPrice <= 0) throw new Error(`No price data for ${ticker}`);
  ...
  if (bars.length >= 10) {
    ...
    for (let i = 1; i < bars.length; i++) {
      if (bars[i - 1].close > 0) gaps.push(Math.abs(bars[i].open - bars[i - 1].close) / bars[i - 1].close);
    }
  ...
  const liquidityScore: PretradeRiskScan["liquidity"]["liquidityScore"] =
    avgDailyValue == null
      ? "LOW"
      : avgDailyValue > 5e9
      ? "HIGH"
  ...
  if (quote?.earningsDate) {
    ...
    if (!isNaN(ms)) earningsInDays = Math.max(0, Math.round((ms - Date.now()) / 86_400_000));
  }
  ...
  const flags: string[] = [];
  let recommendation: PretradeRiskScan["recommendation"] = "PROCEED";

  if (avgDailyValue != null && maxNotional > avgDailyValue * 0.1) {
    recommendation = "REDUCE_SIZE";
    flags.push(
      `Planned notional (₹${(maxNotional / 1e7).toFixed(1)} Cr) exceeds 10% of average daily turnover — may be hard to fill without slippage.`,
    );
  }
  ...
  if (earningsInDays != null && earningsInDays <= 7) {
    ...
    if (recommendation === "PROCEED" || recommendation === "PROCEED_WITH_STOP") recommendation = "PROCEED_WITH_STOP";
  }
