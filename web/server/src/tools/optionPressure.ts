...
// Options positioning & dealer-hedging pressure for an NSE F&O stock, computed
// natively from the option chain. `max_pain` reuses the chain summary; the rest
// are derived pressure metrics.
//
// NOTE: NSE publishes OI + premium + IV per strike but not per-strike Greeks, so
// the gamma wall is a transparent approximation from OI weighted by a
// moneyness-shaped gamma curve (gamma peaks at-the-money and decays away).
...
export interface OptionPressure {
  ...
  maxPain: number | null;
  gammaWall: { strike: number; exposure: number } | null;
  expectedMove: {
    points: number | null;
    pct: number | null;
    method: "straddle" | "unknown";
  };
  squeezeTargets: { up: number | null; down: number | null } | null;
...
export async function getOptionPressure(args: OptionPressureArgs): Promise<OptionPressure> {
  ...
  if (!chain || !chain.strikes.length || !chain.underlying) {
    notes.push(
      "Option chain unavailable (non-F&O stock, or NSE blocked programmatic access). Option-pressure metrics require an active chain.",
    );
    return {
      symbol: ticker,
      underlying: null,
  ...
  if (expiryDate) {
    ...
    if (expMs != null) daysToExpiry = Math.max(0, Math.round((expMs - Date.now()) / 86_400_000));
  }
  ...
  const expectedMove = {
    ...
    method: straddle > 0 ? ("straddle" as const) : ("unknown" as const),
  };
  ...
  let gammaWall: { strike: number; exposure: number } | null = null;
