...
const techCache = new Map<string, { at: number; data: Technical }>();
...
function stochastic(highs: number[], lows: number[], closes: number[], period = 14, signalP = 3) {
  const n = closes.length;
  if (n < period) return { k: NaN, d: NaN };
  const pctK = (j: number) => {
    const c = closes[j];
    let hh = -Infinity;
    let ll = Infinity;
  ...
  const k = pctK(n - 1);
  const ks: number[] = [];
  for (let j = n - signalP; j < n; j++) ks.push(pctK(j));
...
export function macd(closes: number[], fast = 12, slow = 26, signalP = 9) {
  ...
  if (start < 0) return { macd: NaN, signal: NaN, hist: NaN };
...
export function adx(highs: number[], lows: number[], closes: number[], period = 14) {
  ...
  const smooth = (a: number[]) => {
    const o = [a[0]];
    for (let i = 1; i < a.length; i++) o.push(o[i - 1] - o[i - 1] / period + a[i]);
    return o;
  };
  const TR = smooth(tr);
  ...
  let adxv = dx[0];
  for (let i = 1; i < dx.length; i++) adxv = (adxv * (period - 1) + dx[i]) / period;
...
export function atr(highs: number[], lows: number[], closes: number[], period = 14): number {
  ...
  for (let i = p; i < tr.length; i++) prev = (prev * (period - 1) + tr[i]) / period;
  return prev;
}

export function bollingerWidth(closes: number[], period = 20, mult = 2): number {
  const n = closes.length;
  if (n < period) return NaN;
...
function histVol(closes: number[], period = 20): number {
  const n = closes.length;
  if (n < period + 1) return NaN;
  const rets: number[] = [];
  for (let i = n - period; i < n; i++) rets.push(Math.log(closes[i] / closes[i - 1]));
...
// Detected pattern with a human-readable note and a "forming" flag (true when
// the pattern involves the most recent, still-in-progress bar).
...
function istParts(): { y: number; m: number; d: number; h: number; min: number; day: number } {
...
function indiaMarketOpenNow(): boolean {
...
export function detectCandlePatterns(
  ...
  const last = bars[n - 1];
  const eps = (b: any) => Math.abs(b.close) * 1e-4 + 1e-9;
  const bodyAbs = (b: any) => Math.abs(b.close - b.open);
  const rng = (b: any) => (b.high - b.low) || eps(b);
  const bull = (b: any) => b.close > b.open;
  const bear = (b: any) => b.close < b.open;
  const bodyFrac = (b: any) => bodyAbs(b) / rng(b);
  const upFrac = (b: any) => (b.high - Math.max(b.open, b.close)) / rng(b);
  const dnFrac = (b: any) => (Math.min(b.open, b.close) - b.low) / rng(b);
  const push = (name: string, type: PatType, note: string) =>
    out.push({ name, type, note, forming });
  ...
  if (bf <= 0.1) {
    ...
    push(
      bull(last) ? "Bullish Marubozu" : "Bearish Marubozu",
      bull(last) ? "BULLISH" : "BEARISH",
      "Full-bodied candle with little or no wicks — strong conviction.",
    );
  } else if (bf <= 0.4 && dnFrac(last) >= 0.55 && upFrac(last) <= 0.25) {
    push(
      trend < 0 ? "Hammer" : "Hanging Man",
      trend < 0 ? "BULLISH" : "BEARISH",
      "Small body at the top, long lower wick — potential reversal.",
    );
  } else if (bf <= 0.4 && upFrac(last) >= 0.55 && dnFrac(last) <= 0.25) {
    push(
      trend > 0 ? "Shooting Star" : "Inverted Hammer",
      trend > 0 ? "BEARISH" : "BULLISH",
      "Small body at the bottom, long upper wick — potential reversal.",
    );
  } else if (
    bf > 0.1 &&
    bf <= 0.45 &&
  ...
  if (n >= 3) {
    ...
    const p = bars[n - 2];
    const l = last;
    const mid = (x: any) => (x.open + x.close) / 2;
...
function detectPatterns(bars: any[]): Pattern[] {
...
const patternCache = new Map<string, { at: number; data: PatternAnalysis }>();
...
export async function fetchBars(ticker: string, interval: string, days: number): Promise<RawBar[]> {
  const now = Math.floor(Date.now() / 1000);
  const p1 = now - Math.max(days, 1) * 24 * 3600;
  const r = await yahooFinance.chart(ticker, {
    period1: p1,
    period2: now,
    interval: interval as "1d" | "15m" | "5m" | "30m" | "60m" | "1h" | "1wk" | "1mo" | "3mo",
  });
  return (r?.quotes ?? [])
    .filter(
      (q: any) =>
...
function findPivots(highs: number[], lows: number[], left: number, right: number) {
  const hi: number[] = [];
  const lo: number[] = [];
...
function dedupPatterns(patterns: DetectedPattern[]): DetectedPattern[] {
...
function detectChartPatterns(p: {
  ...
  const out: DetectedPattern[] = [];
  const n = closes.length;
  const H = (i: number) => highs[i];
  const L = (i: number) => lows[i];
  const tol =
    Math.max(0.012, (atr / Math.max(last, 1e-9))) * (horizon === "intraday" ? 2 : 1.4);
  ...
  const push = (
    ...
    confidence: PatternConfidence,
    forming = liveLast,
  ) => out.push({ name, type, note, forming, confidence });
      ...
      if (midLo.length) {
        ...
        if (last < neck) {
          push(
            "Double Top",
            "BEARISH",
            `Twin peaks near ${rupee(pa)} with a neckline at ${rupee(neck)}. The neckline break confirms distribution; measured move targets ${rupee(neck - (pa - neck))}.`,
            "HIGH",
            false,
          );
          ...
          push(
            "Double Top (forming)",
            "BEARISH",
            `Two failed attempts to clear ${rupee(pa)}; supply is stepping in at the same shelf. Bearish while below it — a neckline break under ${rupee(neck)} triggers the move.`,
            "MEDIUM",
            true,
          );
        }
      ...
      if (midHi.length) {
        ...
        if (last > neck) {
          push(
            "Double Bottom",
            "BULLISH",
            `Twin lows near ${rupee(pa)} with a neckline at ${rupee(neck)}. The breakout above confirms accumulation; measured target ${rupee(neck + (neck - pa))}.`,
            "HIGH",
            false,
          );
          ...
          push(
            ...
            "BULLISH",
            `Two defensive lows holding at ${rupee(pa)} — buyers are defending the floor. A close above ${rupee(neck)} confirms the base and opens the retest-of-range move.`,
            "MEDIUM",
            true,
          );
        }
  ...
  if (Hn.length >= 3) {
    ...
    if (hd > hs * 1.01 && hd > rs * 1.01 && Math.abs(hs - rs) <= tol * 2 * hs) {
      ...
      if (nl.length >= 2) {
        ...
        const y1 = L(x1);
        const y2 = L(x2);
        const neckAt = (x: number) => (x1 === x2 ? y1 : y1 + ((y2 - y1) * (x - x1)) / (x2 - x1));
        const neckNow = neckAt(i3);
        if (last < neckNow) {
          push(
            "Head & Shoulders",
            "BEARISH",
            `Left shoulder ${rupee(hs)}, head ${rupee(hd)}, right shoulder ${rupee(rs)}; neckline ${rupee(neckNow)} broken. Distribution confirmed — target ≈ ${rupee(neckNow - (hd - neckNow))}.`,
            "HIGH",
            false,
          );
        } else {
          push(
            ...
            "BEARISH",
            `Right shoulder building under the head high of ${rupee(hd)}. Bearish while price holds below the neckline at ${rupee(neckNow)}; a breakdown there triggers the pattern.`,
            "MEDIUM",
            true,
          );
        }
      ...
      if (nh.length >= 2) {
        ...
        if (last > neckNow) {
          push(
            ...
            "BULLISH",
            `Left ${rupee(ls)}, head ${rupee(hd)}, right ${rupee(rs)}; neckline ${rupee(neckNow)} taken out — bottom reversal confirmed, target ≈ ${rupee(neckNow + (neckNow - hd))}.`,
            "HIGH",
            false,
          );
        } else {
          push(
            ...
            "BULLISH",
            `Right shoulder carving above the head low of ${rupee(hd)}. A close over the neckline at ${rupee(neckNow)} confirms the reversal.`,
            "MEDIUM",
            true,
          );
        }
  ...
  if (H5.length >= 3 && L5.length >= 3) {
    ...
    if (Math.abs(hsR) < flatThresh && lsR > flatThresh) {
      push(
        "Ascending Triangle",
        "BULLISH",
        `Flat resistance near ${rupee(top)} with higher lows — a bullish coil. Breakout above resistance (on volume) is the trigger; failure leaks back into the range.`,
        "MEDIUM",
        liveLast,
      );
      ...
      push(
        "Descending Triangle",
        "BEARISH",
        `Flat support near ${rupee(bot)} with lower highs — supply building. A breakdown below support is the trigger; a hold there can fake-and-pop.`,
        "MEDIUM",
        liveLast,
      );
      ...
      push(
        ...
        "NEUTRAL",
        `Converging highs and lows — a coil with no directional bias of its own. Read it as a continuation of the prior ${trendUp ? "uptrend" : "downtrend"}; trade the breakout, not the apex.`,
        "LOW",
        liveLast,
      );
      ...
      push(
        "Rising Wedge",
        "BEARISH",
        `Both highs and lows rising, but lows climb faster — a narrowing up-channel that typically resolves down. Watch for loss of the lower trendline.`,
        "MEDIUM",
        liveLast,
      );
      ...
      push(
        "Falling Wedge",
        "BULLISH",
        `Both highs and lows falling, but narrowing — a falling wedge that typically resolves up. Watch the upper trendline for the breakout.`,
        "MEDIUM",
        liveLast,
      );
    }
  ...
  if (closes.length >= win + 4) {
    ...
    if (Math.abs(poleMove) / (pole[0] || 1) > minPole) {
      ...
      if (opposite && contained) {
        push(
          up ? "Bull Flag" : "Bear Flag",
          up ? "BULLISH" : "BEARISH",
          `A ~${((Math.abs(poleMove) / (pole[0] || 1)) * 100).toFixed(1)}% ${up ? "rally" : "selloff"} sets the pole; price now coils in a tight ${up ? "counter-downtrend" : "counter-uptrend"} flag. Continuation ${up ? "higher" : "lower"} on a break of ${rupee(flag[flag.length - 1])}.`,
          "MEDIUM",
          liveLast,
        );
      }
      ...
      if (depth >= 0.08 && depth <= 0.6 && Math.abs(left - right) / left <= 0.05) {
        ...
        if (handlePull < depth * 0.5) {
          push(
            "Cup & Handle",
            "BULLISH",
            `A ${depth > 0.2 ? "deep" : "shallow"} rounding base over ${cwin} bars with rims near ${rupee(left)} and a shallow handle — textbook accumulation. Breakout above ${rupee(handleMax)} targets ${rupee(right + depth * left)}.`,
            "MEDIUM",
            liveLast,
          );
        }
  ...
  if (H5.length >= 3 && L5.length >= 3) {
    ...
    if (topR < flatThresh && botR < flatThresh && (rangeH - rangeL) / avg < 0.15) {
      push(
        "Rectangle (Range)",
        "NEUTRAL",
        `Price boxed between ${rupee(rangeL)} support and ${rupee(rangeH)} resistance. No trend edge until one gives — trade the range or wait for the breakout and follow it.`,
        "LOW",
        liveLast,
      );
    }
...
function analyzeHorizon(
  ...
  if (horizon !== "intraday" && bars.length < 30) return [];
  ...
  const candle = detectCandlePatterns(bars.slice(-3), { forming: liveLast }).map(
    (c) => ({ ...c, confidence: "MEDIUM" as PatternConfidence }),
  );

  return dedupPatterns([...chart, ...candle]).slice(0, 8);
}

export async function getPatternAnalysis(args: TechnicalArgs): Promise<PatternAnalysis> {
  ...
  if (cached && Date.now() - cached.at < PATTERN_TTL_MS) return cached.data;

  const [intraday, short, long] = await Promise.all([
    fetchBars(ticker, "15m", 1).catch(() => [] as RawBar[]),
    fetchBars(ticker, "1d", 90).catch(() => [] as RawBar[]),
    fetchBars(ticker, "1d", 365).catch(() => [] as RawBar[]),
  ]);

  const open = indiaMarketOpenNow();
  const rows: PatternAnalysis = [
    {
      horizon: "intraday",
      label: "Intraday · 15m",
      interval: "15m",
...
function band(val: number, buyBelow: number, sellAbove: number): Signal {
  if (Number.isNaN(val)) return "NEUTRAL";
  if (val <= buyBelow) return "BUY";
  if (val >= sellAbove) return "SELL";
  return "NEUTRAL";
}

const scoreOf = (s: Signal) => (s === "BUY" ? 1 : s === "SELL" ? -1 : 0);

function summaryLabel(score: number): TechnicalSummary["label"] {
...
const fP = (n: number) => (Number.isNaN(n) ? "—" : `₹${n.toLocaleString("en-IN", { maximumFractionDigits: 2 })}`);
...
function scoreSignal(
  ...
  const macdSignal: Signal = Number.isNaN(macdV.macd)
    ? "NEUTRAL"
  ...
  const adxSignal: Signal =
    Number.isNaN(adxV.adx) || adxV.adx < 20
      ? "NEUTRAL"
      : adxV.plusDI > adxV.minusDI
      ? "BUY"
      : "SELL";

  const oscillators: TechSignal[] = [
  ...
  const movingAverages: TechSignal[] = [];
...
// ===========================================================================
// Confluence-level engine.
//
// No single indicator gives an exact buy/sell price. Instead we gather
// candidate levels from six *independent* tools — swing pivots, round
// numbers, Fibonacci retracements/extensions, moving averages, VWAP and the
// volume-profile POC — then cluster the candidates that land near each other.
// A cluster's `strength` is how many distinct tools agree on that price; the
// strongest cluster below the current price is the buy zone, the strongest
// above is the sell zone. Stops/targets come from ATR and the opposite zone.
// ===========================================================================
...
function roundNumStep(p: number): number {
  if (p < 50) return 1;
  if (p < 100) return 2;
  if (p < 500) return 5;
  if (p < 1000) return 10;
...
export function computeConfluence(
  ...
  price: number,
  atr: number,
  bias: ConfluencePlan["bias"],
): ConfluencePlan {
  const empty: ConfluencePlan = {
    bias,
    buyZone: null,
    sellZone: null,
    longStop: null,
  ...
  if (closes.length < 30 || !Number.isFinite(price) || !(atr > 0)) return empty;
  ...
  const cands: { price: number; tool: ConfluenceTool }[] = [];

  // 1. Swing pivots (left=right=3) over the window.
  const { hi: pHi, lo: pLo } = findPivots(cHighs, cLows, 3, 3);
  for (const i of pLo) cands.push({ price: cLows[i], tool: "Pivot" });
  for (const i of pHi) cands.push({ price: cHighs[i], tool: "Pivot" });
  ...
  if (cumV > 0) cands.push({ price: cumPV / cumV, tool: "VWAP" });
  ...
  let pocB = 0;
  for (let b = 1; b < bins; b++) if (bVol[b] > bVol[pocB]) pocB = b;
  ...
  for (const c of sorted) {
    ...
    if (last && c.price <= last.high + tol) {
      ...
      last.count++;
      if (!last.sources.includes(c.tool)) last.sources.push(c.tool);
    } else {
      raw.push({ price: c.price, low: c.price, high: c.price, sources: [c.tool], count: 1 });
    }
  }

  const levels: ConfluenceLevel[] = raw.map((r) => ({
    price: r.price,
    low: r.low,
    high: r.high,
  ...
  let longStop: number | null = null;
  let longTarget: number | null = null;
  let longRRR: number | null = null;
  if (buyZone) {
...
export async function getTechnicalAnalysis(args: TechnicalArgs): Promise<Technical> {
  const raw = (args.symbol ?? "").trim();
  if (!raw) throw new Error("symbol is required");
  const upper = raw.toUpperCase();
  const ticker = /\.[A-Z]{1,3}$/.test(upper) ? upper : `${upper}.NS`;
  ...
  if (cached && Date.now() - cached.at < TECH_TTL_MS) return cached.data;
  ...
  const bars = (r?.quotes ?? []).filter(
    (q: any) => q && typeof q.close === "number" && q.high != null && q.low != null
  );
  if (bars.length < 30) {
  ...
  const vols = bars.map((b: any) => (b.volume ?? 0) as number);
  ...
  const triggerDateRaw = (bars[triggerIndex] as { date?: Date | string } | undefined)?.date;
  const triggerDate =
    triggerDateRaw != null
  ...
  const confluence = computeConfluence(
    ...
    price,
    volatility.atr,
    sig.label.includes("BUY") ? "BUY" : sig.label.includes("SELL") ? "SELL" : "NEUTRAL",
  );
