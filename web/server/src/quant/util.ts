...
export function normalizeSymbol(raw: string): string {
  ...
  if (/^[0-9]{4,6}$/.test(upper)) return `${upper}.BO`;
  return `${upper}.NS`;
}
...
export function validateSymbol(raw: string): string {
  ...
  if (!SYMBOL_RE.test(base)) throw new Error(`invalid symbol: ${JSON.stringify(raw)}`);
  return ticker;
}
...
export async function fetchDailyBars(
  ticker: string,
  days: number,
): Promise<{ ticker: string; bars: DailyBar[] }> {
...
export function annualizedVol(closes: number[], periods = 252): number {
