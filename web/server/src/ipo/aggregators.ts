...
function aggConfig(name: string): SourceConfig {
  return {
    name,
    userAgent: AGG_UA,
...
function matchRecord(list: any[], symbol?: string | null, name?: string | null): any | null {
...
function parseSubscription(v: any): IpoSubscription | null {
  if (!v) return null;
  return {
...
function parseFinancials(v: any): IpoFinancials | null {
  if (!v) return null;
  return {
...
function parseInvestorGain(rec: any): GmpEnrichment {
  return {
...
const investorGainSource: GmpSource = {
  name: "InvestorGain",
  async fetch(symbol, companyName) {
    for (const path of INVESTORGAIN_PATHS) {
      ...
      if (rec) return parseInvestorGain(rec);
    }
    return null;
  },
};
...
function extractGmpFromHtml(html: string, companyName?: string | null): number | null {
  ...
  for (const row of rows) {
    if (!norm(row).includes(nn)) continue;
...
async function htmlGmpSource(name: string, url: string, symbol?: string | null, companyName?: string | null): Promise<GmpEnrichment | null> {
...
const ipoWatchSource: GmpSource = {
  name: "IPO Watch",
  fetch: (symbol, companyName) =>
    htmlGmpSource("IPO Watch", "https://www.ipowatch.in/ipo-gmp/", symbol, companyName),
};

const chittorgarhSource: GmpSource = {
  name: "Chittorgarh",
  fetch: (symbol, companyName) =>
    htmlGmpSource("Chittorgarh", "https://www.chittorgarh.com/ipo/", symbol, companyName),
};
...
const SOURCES: GmpSource[] = [investorGainSource, ipoWatchSource, chittorgarhSource];

export async function fetchEnrichment(
  symbol?: string | null,
  companyName?: string | null
): Promise<GmpEnrichment | null> {
  for (const s of SOURCES) {
    try {
      const e = await s.fetch(symbol, companyName);
      if (e && (e.gmp !== null || e.subscription || e.financials)) return e;
    } catch {
      // try next source
    }
  }
